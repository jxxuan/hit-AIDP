...
class TextCNN(nn.Module):
    def __init__(self, num_labels):
        ...
        self.cnn = nn.Conv2d(
            in_channels=1,
            out_channels=128,
            kernel_size=(3, self.bert_config.hidden_size)
        )
    ...
    def forward(self, input_ids):
        ...
        # print(outputs.last_hidden_state.shape)    [64, 300, 768]
        pooled_output = outputs.last_hidden_state  # .permute(0, 2, 1)  # 使用pooled_output作为输入

        pooled_output = pooled_output.unsqueeze(1)  # 增加一个维度，以适应Conv1d的输入形状 [batch_size, hidden_size, sequence_length]
        cnn_output = self.cnn(pooled_output)    # [64, 128, 298 1]
        # print(cnn_output.shape)
        cnn_output = self.dropout(cnn_output)
        print(cnn_output.shape)
        fc_input = cnn_output  # .permute(0, 2, 1)
